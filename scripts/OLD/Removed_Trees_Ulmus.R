...
dir.base <- "Google Drive/My Drive/LivingCollections_Phenology/Observing Lists"

path.google <- "~/GoogleDrive/My Drive" # Mac
path.out <- "~/Google Drive/My Drive/LivingCollections_Phenology/Observing Lists/Ulmus/"

#Batch loading trees from the Ulmus tree observation lists and placing them in data frame dat.all
dat.all <- read_bulk(directory = "../LivingCollections_Phenology/Observing Lists/Ulmus", extension = ".csv", header= TRUE,)
head(dat.all)
...
new.dat <- anti_join(dat.all, df.gone, by=("PlantNumber"), row.names=F)

#deleting duplicates
new.dat <- arrange(new.dat, Obs.List)
new.dat <- new.dat[!duplicated(new.dat$PlantNumber), ]
summary(new.dat)

head(new.dat)

#Writing a csv of new.dat to be our Ulmus all list

write.csv(new.dat, paste0(path.out, "ObservingList_Ulmus.csv"), row.names = F)
...
for (Obs.List in names(split_new.dat)) {
  write.csv(split_new.dat[[Obs.List]], paste0(path.out,"ObservingList_Ulmus_",Obs.List,".csv"), row.names = F,)
}
