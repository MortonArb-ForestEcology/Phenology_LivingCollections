...
# bind missing trees back into 2026 lists
q26update <- rbind(q26, notqbr26)
u26update <- rbind(u26, notubr26)
a26update <- rbind(a26, notabr26)
...
#write list of all 2026 obs lists out
write_csv(q26update, file = file.path(path.google, "Observing Lists", "Quercus", "ObservingList_Quercus_2026.csv"))
write_csv(u26update, file = file.path(path.google, "Observing Lists", "Ulmus", "ObservingList_Ulmus_2026.csv"))
write_csv(a26update, file = file.path(path.google, "Observing Lists", "Acer", "ObservingList_Acer_2026.csv"))

#Write individual lists out 
for(OBSLIST in unique(q26update$List)){
  write.csv(q26update[q26update$List==OBSLIST,], file=file.path(path.google, "Observing Lists", "Quercus", paste0("ObservingList_Quercus_2026_LIST-", stringr::str_pad(OBSLIST, 2, "left", "0"), ".csv")), row.names=F)
}

for(OBSLIST in unique(a26update$List)){
  write.csv(a26update[a26update$List==OBSLIST,], file=file.path(path.google, "Observing Lists", "Acer", paste0("ObservingList_Acer_2026_LIST-", stringr::str_pad(OBSLIST, 2, "left", "0"), ".csv")), row.names=F)
}

for(OBSLIST in unique(u26update$List)){
  write.csv(u26update[u26update$List==OBSLIST,], file=file.path(path.google, "Observing Lists", "Ulmus", paste0("ObservingList_Ulmus_2026_LIST-", stringr::str_pad(OBSLIST, 2, "left", "0"), ".csv")), row.names=F)
}

# ~~~~~~~~~~~~~~~~
# Combine all lists and put them in the PhenoApp Folder
# ~~~~~~~~~~~~~~~~
listsAll <- rbind(q26update, a26update, u26update)
