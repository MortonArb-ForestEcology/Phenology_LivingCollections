...
YearExtremes <- 
  ...
  theme_minimal()
YearExtremes
# #practice saving this graph to html: not working
# library(htmlwidgets)
# dir.create("Z:\\new folder")
...
agg_mean <- aggregate(dat.ghcn[5:6], by=list(dat.ghcn$MONTH), FUN=mean)
  names(agg_mean)[names(agg_mean) == "Group.1"] <- "MONTH"
  agg_mean
...
#practice with the merge function: fixed
merge(x=agg_mean2, y=agg_mean)
  

#practice with the stack function
stack_df <- stack(agg_mean2, select=c("PRCP", "SNOW"))
unstack(stack_df)
...
Atts <- yearly_Textremes %>%
  select(YearlyTempMax, YearlyTempMin) %>%
  names() 
Atts

Ctrs <- unique(yearly_Textremes$Year)
Ctrs
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput(inputId = "selects", choices = Ctrs,
                  label = "Select Year", multiple = TRUE),
      selectInput(inputId = "selects2", choices = Atts, label = "select extreme",
                  multiple = TRUE)
    ),
...
server <- function(input, output) {
  ...
  output$Plot <- renderPlotly({
    ...
    print(ggplotly(ggplot(data = data, aes(x = grp, y= value)) + 
      geom_bar(aes(group = Attribute, fill = Year, 
                   # text = sprintf("Group: %s", paste(grp)) #did not work when I tried to use this to add extra hover info
...
#copied from internet program which filters which data is being used: not working
library(shiny)
library(shinythemes)
library(dplyr)
library(DT)
